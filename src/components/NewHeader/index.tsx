...
export function NewHeader() {
  ...
  return (
    <>
      ...
      >
        <div className={`${!hasScrolled && 'relative flex justify-between w-[80%] m-auto'}`}>
          <div className={`w-[55%] flex justify-between items-center transition-all ease-in-out duration-500 ${hasScrolled && 'w-[90%]'}`}>
            ...
            <nav
              className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-12 items-center text-purple-950 h-fit text-[16px] tracking-wide font-semibold transition-all ease-in-out duration-700
          ...
          {!hasScrolled && (
            <div>
              <Link
                ...
                download={fileName}
                target="_blank"
                className="text-gray-800 transition-all ease-in duration-100 font-normal border border-gray-200 p-3 rounded-lg text-[1rem] tracking-wide"
              >
                Press <span className=" bg-gray-200 border border-gray-300 rounded-[3px] px-[4px] pt-[3px]">B</span> to Book Call
              </Link>
            </div>
          )}
        </div>
    ...
    // >
    //   <div className={`w-[90%] flex justify-between items-center transition-all ease-in-out duration-500 ${hasScrolled && 'w-[90%]'}`}>
    //     {!hasScrolled && (
    //       <Link href={'#'} className="text-3xl text-black font-bold tracking-tight">
    //         nawaz<span className="text-purple-900">.</span>
    //       </Link>
    //     )}
    //     <nav className={`flex justify-center w-full pt-0 gap-12 items-center text-purple-950 h-fit text-[16px] tracking-wide font-semibold transition-all ease-in-out duration-700`}>
    //       <Link href={'#'} onClick={() => (document.body.scrollTop = 0)} className="hover:underline underline-offset-8 decoration-1">
    //         Home
    //       </Link>
    //       <Link href={'#about'} className="hover:underline underline-offset-8 decoration-1">
