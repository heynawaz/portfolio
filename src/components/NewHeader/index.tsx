  ...
  return (
    ...
    >
      <div className={`w-[90%] flex justify-between items-center transition-all ease-in-out duration-500 ${hasScrolled && 'w-[90%]'}`}>
        ...
        <nav
          className={`${hasScrolled ? 'flex' : 'hidden'} justify-center w-full pt-0 gap-12 items-center text-purple-950 h-fit text-[16px] tracking-wide font-semibold transition-all ease-in-out duration-700`}
        >
          <Link href={'#'} onClick={() => (document.body.scrollTop = 0)} className="hover:underline underline-offset-8 decoration-1">
            Home
          </Link>
