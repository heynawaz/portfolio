  ...
  return (
    <div ref={containerRef} className={cn('scroller relative z-20 max-w-[60vw] overflow-hidden  [mask-image:linear-gradient(to_right,transparent,white_20%,white_80%,transparent)]', className)}>
      <ul ref={scrollerRef} className={cn(' flex min-w-full shrink-0 gap-4 py-4 w-max flex-nowrap', start && 'animate-scroll ', pauseOnHover && 'hover:[animation-play-state:paused]')}>
        {items.map((item, idx) => (
          <li className="w-[350px] max-w-full relative rounded-2xl border border-gray-200 flex-shrink-0 px-8 py-6 md:w-[450px] bg-neutral-50/95" key={item.name}>
            <blockquote>
              ...
              <div className="relative z-20 mt-6 flex flex-row gap-4 items-center">
                <Image src={'/assets/Shah Nawaz.jpg'} draggable={false} className="rounded-full" alt="Shah Nawaz's Photo" width={45} height={40} />
