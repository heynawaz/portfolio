...
export default function AnimatedGradientText({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        'group relative mx-auto sm:mx-0 flex max-w-fit flex-row items-center justify-center rounded-full bg-white/40 px-4 py-1 pt-[7px] text-[1rem] sm:text-[.7rem] font-medium shadow-[inset_0_-8px_10px_#8fdfff1f] backdrop-blur-sm transition-shadow duration-500 ease-out [--bg-size:300%] hover:shadow-[inset_0_-5px_10px_#8fdfff3f]',
        className,
      )}
    >
      <div
